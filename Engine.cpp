...
Engine::Engine(sf::RenderWindow &win,bool b)
{
	...
	//napisy
	if(!font.loadFromFile("data/horrendo.ttf"))
	{
		cout<<"Error";
		return;
	};
...
void Engine::runEngine()
{
	...
	while(!menu)
	{
		...
		{
			bullets_player.add(player.getAngle(),player.getPosition(0));
...
void Engine::update()
{
	...
	if(Enemy.getSize() <= 0)
	{
		...
		Enemy.generate(liczba_asteroid);
...
void Engine::collision()
{
	...
	if(!Enemy.isEmpty())
	{
		...
		int numAsteroid = 0;
		while(numAsteroid < Enemy.getSize())
		{
			Vector2f *B = new Vector2f[Enemy.getSize(numAsteroid)];

			for(int i=0;i<Enemy.getSize(numAsteroid);i++)
				B[i] = Enemy.getPoint(numAsteroid,i);
	
			bool collision;
		
			for(int i=0;i<3;i++)
			{
				for(int j=0;j<Enemy.getSize(numAsteroid);j++)
				{
					collision=(check_collision(A,3,B,Enemy.getSize(numAsteroid),A[i]-B[j]));
					if(collision)
					{
						break;
					}
	...
	if(!bullets_player.isEmpty())
	{
		...
		for(int i=0;i<bullets_player.getSize();i++)
		{
			...
			{
				Vector2f *vert = new Vector2f[Enemy.getSize(j)];
				for(int k=0;k<Enemy.getSize(j);k++)
				{
					vert[k] = Enemy.getPoint(j,k);
				}
				
				bool czy_w_polygonie = pointInside(bullets_player.getPoint(i),vert,Enemy.getSize(j));
